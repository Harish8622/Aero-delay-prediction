...
def _compute_flags_from_raw(
    ...
    # ✨ Normalize timestamp: UTC -> naive (no tz), floor to hour
    out["ts"] = (
        pd.to_datetime(out["ts"], errors="coerce", utc=True)
        .dt.tz_convert(None)
        .dt.floor("h")
    )
...
def _merge_flags(df: pd.DataFrame, wx_flags: pd.DataFrame) -> pd.DataFrame:
    """Merge dep/arr flags into flights by (IATA, hour-key) without *_missing columns."""
    ...
    out["dep_rain"], out["dep_ice"], out["dep_wind"] = (
        dep["dep_rain"].fillna(0).astype("int8"),
        dep["dep_ice"].fillna(0).astype("int8"),
        dep["dep_wind"].fillna(0).astype("int8"),
    )
    out["arr_rain"], out["arr_ice"], out["arr_wind"] = (
        arr["arr_rain"].fillna(0).astype("int8"),
        arr["arr_ice"].fillna(0).astype("int8"),
        arr["arr_wind"].fillna(0).astype("int8"),
    )
